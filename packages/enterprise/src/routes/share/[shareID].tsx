import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@kellton-kratos/sdk/v2"
import { SessionTurn } from "@kellton-kratos/ui/session-turn"
import { SessionReview } from "@kellton-kratos/ui/session-review"
import { DataProvider } from "@kellton-kratos/ui/context"
import { DiffComponentProvider } from "@kellton-kratos/ui/context/diff"
import { CodeComponentProvider } from "@kellton-kratos/ui/context/code"
import { WorkerPoolProvider } from "@kellton-kratos/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createEffect, createMemo, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@kellton-kratos/ui/logo"
import { IconButton } from "@kellton-kratos/ui/icon-button"
import { ProviderIcon } from "@kellton-kratos/ui/provider-icon"
import { createDefaultOptions } from "@kellton-kratos/ui/pierre"
import { iife } from "@kellton-kratos/util/iife"
import { Binary } from "@kellton-kratos/util/binary"
import { NamedError } from "@kellton-kratos/util/error"
import { DateTime } from "luxon"
import { SessionMessageRail } from "@kellton-kratos/ui/session-message-rail"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@kellton-kratos/ui/tabs"
import { preloadMultiFileDiff, PreloadMultiFileDiffResult } from "@pierre/diffs/ssr"
import { Diff as SSRDiff } from "@kellton-kratos/ui/diff-ssr"
import { clientOnly } from "@solidjs/start"
import { type IconName } from "@kellton-kratos/ui/icons/provider"
import { Meta, Title } from "@solidjs/meta"
import { Base64 } from "js-base64"

const ClientOnlyDiff = clientOnly(() => import("@kellton-kratos/ui/diff").then((m) => ({ default: m.Diff })))
const ClientOnlyCode = clientOnly(() => import("@kellton-kratos/ui/code").then((m) => ({ default: m.Code })))
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@kellton-kratos/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/kratos-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Kellton Kratos</Title>
              </Show>
              <Meta name="description" content="kratos - The AI coding agent built for the terminal." />
                        ...
                        return (
                          <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                            <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                              <div class="">
                                <a href="https://kellton-kratos.io">
                                  <Mark />
                                </a>
                              </div>
                              <div class="flex gap-3 items-center">
                                <IconButton
                                  as={"a"}
                                  href="https://github.com/RahulSinghai606/kellton-kratos"
                                  target="_blank"
                                  icon="github"
                                  variant="ghost"
                                />
                                <IconButton
                                  as={"a"}
                                  href="https://kellton-kratos.io/discord"
                                  target="_blank"
                                  icon="discord"
                                  variant="ghost"
                                />
