...
Create `.kratos/command/test.md`:

```md title=".kratos/command/test.md"
---
...
You can add custom commands through the Kellton Kratos config or by creating markdown files in the `command/` directory.

---

### JSON

Use the `command` option in your Kellton Kratos [config](/docs/config):

```json title="kratos.jsonc" {4-12}
{
...
- Global: `~/.config/kratos/command/`
- Per-project: `.kratos/command/`

```markdown title="~/.config/kratos/command/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".kratos/command/component.md"
---
...
```md title=".kratos/command/create-file.md"
---
...
```md title=".kratos/command/analyze-coverage.md"
---
...
```md title=".kratos/command/review-changes.md"
---
...
```md title=".kratos/command/review-component.md"
---
...
kratos includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
