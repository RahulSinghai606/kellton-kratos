...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start Kellton Kratos.
...
Here are several models that work well with Kellton Kratos, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Kellton Kratos config.

```json title="kratos.json" {3}
{
...
Here the full ID is `provider_id/model_id`. For example, if you're using [Kellton Kratos Zen](/docs/zen), you would use `kratos/gpt-5.1-codex` for GPT 5.1 Codex.
...
```jsonc title="kratos.jsonc" {7-12,19-24}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "openai": {
...
Many models support multiple variants with different configurations. Kellton Kratos ships with built-in default variants for popular providers.

### Built-in variants

Kellton Kratos ships with default variants for many providers:
...
When Kellton Kratos starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the Kellton Kratos config.
