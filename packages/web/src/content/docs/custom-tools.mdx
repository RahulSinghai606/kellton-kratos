---
title: Custom Tools
description: Create tools the LLM can call in kratos.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside kratos's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.kratos/tool/` directory of your project.
- Or globally, by placing them in `~/.config/kratos/tool/`.
...
```ts title=".kratos/tool/database.ts" {1}
...
```ts title=".kratos/tool/project.ts" {8}
import { tool } from "@kellton-kratos/plugin"
...
```python title=".kratos/tool/add.py"
import sys
...
```ts title=".kratos/tool/python-add.ts" {10}
import { tool } from "@kellton-kratos/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .kratos/tool/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
