---
title: Server
description: Interact with kratos server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `kratos serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an kratos client can use.
...
```bash
kratos serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
kratos serve --cors http://localhost:5173 --cors https://app.example.com
```
...
When you run `kratos` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the kratos server to interact with kratos programmatically.
:::

This architecture lets kratos support multiple clients and allows you to interact with kratos programmatically.

You can run `kratos serve` to start a standalone server. If you have the
kratos TUI running, `kratos serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Kellton Kratos [IDE](/docs/ide) plugins.
...
The kratos server exposes the following APIs.
