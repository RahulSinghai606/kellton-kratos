---
title: Providers
description: Using any LLM provider in Kellton Kratos.
---

import config from "../../../config.mjs"
export const console = config.console

Kellton Kratos uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support for **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Kellton Kratos config.
...
in `~/.local/share/kratos/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Kellton Kratos
config.
...
Kellton Kratos Zen is a list of models provided by the Kellton Kratos team that have been
tested and verified to work well with Kellton Kratos. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Kellton Kratos Zen.
:::

1. Run the `/connect` command in the TUI, select kratos, and head to [kellton-kratos.io/auth](https://kellton-kratos.io/auth).
...
It works like any other provider in Kellton Kratos. And is completely optional to use
it.
...
To use Amazon Bedrock with Kellton Kratos:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running kratos:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY kratos

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile kratos

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX kratos
   ```
   ...
   For project-specific or persistent configuration, use `kratos.json`:

   ```json title="kratos.json"
   {
     "$schema": "https://kellton-kratos.io/config.json",
     "provider": {
       "amazon-bedrock": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for kratos to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX kratos
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX kratos
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your kratos config.
...
To use your GitHub Copilot subscription with kratos:
...
To use Google Vertex AI with Kellton Kratos:
...
2. Set the required environment variables:
   ...
   Set them while running kratos.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id kratos
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through kratos, you can always configure it yourself.
...
```jsonc title="~/.config/kratos/kratos.jsonc"
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [kratos-helicone-session](https://github.com/H2Shami/kratos-helicone-session) plugin to automatically log each Kellton Kratos conversation as a session in Helicone.

```bash
npm install -g kratos-helicone-session
```
...
```json title="kratos.json"
{
  "plugin": ["kratos-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Kellton Kratos conversation listed as a separate session.
...
You can configure kratos to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="kratos.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure kratos to use local models through LM Studio.

```json title="kratos.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "lmstudio": {
...
You can configure kratos to use local models through Ollama.

```json title="kratos.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Kellton Kratos:
...
3. Copy the API key for use in Kellton Kratos.
...
6. **Important**: Before using cloud models in Kellton Kratos, you must pull the model information locally:
...
Kellton Kratos Zen is a list of tested and verified models provided by the Kellton Kratos team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Kellton Kratos Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Kellton Kratos Zen**.

   ```txt
   /connect
   ```

3. Enter your Kellton Kratos API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your kratos config.
...
5. You can also customize them through your kratos config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' kratos
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group kratos
   ```
...
You can also customize models through your kratos config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with kratos. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in kratos.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `kratos.json` file in your project directory:

   ```json title="kratos.json" ""myprovider"" {5-15}
   {
     "$schema": "https://kellton-kratos.io/config.json",
     "provider": {
       "myprovider": {
...
```json title="kratos.json" {9,11,17-20}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Kellton Kratos to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `kratos auth list` to see if the credentials
...
2. For custom providers, check the kratos config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your kratos config.
