...
There are two types of agents in Kellton Kratos; primary agents and subagents.
...
Kellton Kratos comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Kellton Kratos comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Kellton Kratos comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `kratos.json` config file:
...
- Global: `~/.config/kratos/agent/`
- Per-project: `.kratos/agent/`

```markdown title="~/.config/kratos/agent/review.md"
---
...
If no temperature is specified, Kellton Kratos uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Kellton Kratos config and the project specific config.
...
The model ID in your Kellton Kratos config uses the format `provider/model-id`. For example, if you're using [Kellton Kratos Zen](/docs/zen), you would use `kratos/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="kratos.json" {3-6,9-12}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "tools": {
    "write": true,
...
```json title="kratos.json" {3-5,8-10}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `kratos models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/kratos).
:::
...
```markdown title="~/.config/kratos/agent/docs-writer.md"
---
...
```markdown title="~/.config/kratos/agent/security-auditor.md"
---
