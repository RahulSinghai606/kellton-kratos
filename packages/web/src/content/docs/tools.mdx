...
Tools allow the LLM to perform actions in your codebase. Kellton Kratos comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
```markdown title="~/.config/kratos/agent/readonly.md"
---
...
Here are all the built-in tools available in Kellton Kratos.
...
:::note
This tool is only available when `KRATOS_EXPERIMENTAL_LSP_TOOL=true` (or `KRATOS_EXPERIMENTAL=true`).
:::
