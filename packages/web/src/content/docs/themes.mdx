...
With Kellton Kratos you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Kellton Kratos uses our own `kratos` theme.
...
Kellton Kratos comes with several built-in themes.
...
- Want Kellton Kratos to match their terminal's appearance
...
Kellton Kratos supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/kratos/themes/*.json` or `$XDG_CONFIG_HOME/kratos/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.kratos/themes/*.json`
4. **Current working directory** - Defined in `./.kratos/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/kratos/themes
vim ~/.config/kratos/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .kratos/themes
vim .kratos/themes/my-theme.json
```
...
{
  "$schema": "https://kellton-kratos.io/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
