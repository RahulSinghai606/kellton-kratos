...
:::caution
Modes are now configured through the `agent` option in the kratos config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in kratos allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the kratos config.
...
kratos comes with two built-in modes.
...
Configure modes in your `kratos.json` config file:
...
- Global: `~/.config/kratos/mode/`
- Project: `.kratos/mode/`

```markdown title="~/.config/kratos/mode/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
If no temperature is specified, kratos uses model-specific defaults (typically 0 for most models, 0.55 for Qwen models).
...
both the global kratos config and the project specific config.
...
Create mode files in `.kratos/mode/` for project-specific modes or `~/.config/kratos/mode/` for global modes:

```markdown title=".kratos/mode/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/kratos/mode/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
