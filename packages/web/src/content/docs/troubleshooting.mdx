...
To debug any issues with Kellton Kratos, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/kratos/log/`
- **Windows**: `%USERPROFILE%\.local\share\kratos\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `kratos --log-level DEBUG`.

---

### Storage

kratos stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/kratos/`
- **Windows**: `%USERPROFILE%\.local\share\kratos`
...
If you're experiencing issues with Kellton Kratos:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/kratos/issues**](https://github.com/anomalyco/kratos/issues)
...
2. **Join our Discord**
   ...
   [**kellton-kratos.io/discord**](https://kellton-kratos.io/discord)
...
### Kellton Kratos won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `kratos upgrade`
...
To figure out what models you have access to, run `kratos models`
...
If you encounter API call errors, this may be due to outdated provider packages. kratos dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart kratos to reinstall the latest provider packages

This will force kratos to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
kratos will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
