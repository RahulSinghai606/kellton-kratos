...
You can add external tools to Kellton Kratos using the _Model Context Protocol_, or MCP. Kellton Kratos supports both local and remote servers.
...
You can define MCP servers in your [Kellton Kratos Config](https://kellton-kratos.io/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="kratos.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/kratos` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Kellton Kratos automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Kellton Kratos will:
...
If the server requires authentication, Kellton Kratos will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `kratos mcp auth <server-name>`.
...
```bash
kratos mcp auth my-oauth-server
```
...
```bash
kratos mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Kellton Kratos will store the tokens securely in `~/.local/share/kratos/mcp-auth.json`.
...
kratos mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Kellton Kratos, alongside built-in tools. So you can manage them through the Kellton Kratos config like any other tool.
...
```json title="kratos.json" {11, 14-18}
{
  "$schema": "https://kellton-kratos.io/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
kratos mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Kellton Kratos to your Sentry account.
