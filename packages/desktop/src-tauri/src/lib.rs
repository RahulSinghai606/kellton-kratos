...
fn get_sidecar_port() -> u32 {
    option_env!("KRATOS_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("KRATOS_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, port: u32) -> CommandChild {
    ...
    let (mut rx, child) = app
        ...
        .sidecar("kratos-cli")
        .unwrap()
        .env("KRATOS_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("KRATOS_CLIENT", "desktop")
        .env("XDG_STATE_HOME", &state_dir)
        .args(["serve", &format!("--port={port}")])
        .spawn()
        .expect("Failed to spawn kratos");
        ...
        .setup(move |app| {
            ...
            let mut window_builder =
                WebviewWindow::builder(&app, "main", WebviewUrl::App("/".into()))
                    ...
                    .initialization_script(format!(
                        r#"
                      window.__KRATOS__ ??= {{}};
                      window.__KRATOS__.updaterEnabled = {updater_enabled};
                      window.__KRATOS__.port = {port};
                    "#
                    ));
...

            {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    let (child, res) = if should_spawn_sidecar {
                        ...
                        let res = loop {
                            if timestamp.elapsed() > Duration::from_secs(7) {
                                break Err(format!(
                                    "Failed to spawn Kellton Kratos Server. Logs:\n{}",
                    ...
                    if res.is_ok() {
                        let _ = window.eval("window.__KRATOS__.serverReady = true;");
                    }
