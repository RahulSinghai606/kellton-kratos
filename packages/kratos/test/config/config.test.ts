...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "kratos.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kratos directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kratosDir = path.join(dir, ".kratos")
      await fs.mkdir(kratosDir, { recursive: true })
      const agentDir = path.join(kratosDir, "agent")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "kratos.json"),
        JSON.stringify({ $schema: "https://kellton-kratos.io/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "kratos.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kratos config
      const projectDir = path.join(dir, "project")
      const kratosDir = path.join(projectDir, ".kratos")
      await fs.mkdir(kratosDir, { recursive: true })
      ...
      // Local .kratos config with different plugins
      await Bun.write(
        path.join(kratosDir, "kratos.json"),
        JSON.stringify({
          $schema: "https://kellton-kratos.io/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kratos config with some overlapping plugins
      await Bun.write(
        path.join(kratosDir, "kratos.json"),
        JSON.stringify({
          $schema: "https://kellton-kratos.io/config.json",
...
test("local .kratos config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kratos directory config enables it
      const kratosDir = path.join(dir, ".kratos")
      await fs.mkdir(kratosDir, { recursive: true })
      await Bun.write(
        path.join(kratosDir, "kratos.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kratos")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/kratos")
