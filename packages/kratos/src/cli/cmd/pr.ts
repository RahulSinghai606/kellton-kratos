...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run kratos",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for kratos session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kratos session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`kratos import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting kratos...")
        UI.println()

        // Launch kratos TUI with session ID if available
        const { spawn } = await import("child_process")
        const kratosArgs = sessionId ? ["-s", sessionId] : []
        const kratosProcess = spawn("kratos", kratosArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
        ...
        await new Promise<void>((resolve, reject) => {
          kratosProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`kratos exited with code ${code}`))
          })
          kratosProcess.on("error", reject)
        })
      },
    })
  },
