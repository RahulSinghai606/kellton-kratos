...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall kratos and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Kellton Kratos")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g kellton-kratos",
      pnpm: "pnpm uninstall -g kellton-kratos",
      bun: "bun remove -g kellton-kratos",
      yarn: "yarn global remove kellton-kratos",
      brew: "brew uninstall kratos",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Kellton Kratos!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      .text()
      .catch(() => "")
    if (content.includes("# kratos") || content.includes(".kratos/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".kratos/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".kratos/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".kratos"))
    ) {
      continue
    }
