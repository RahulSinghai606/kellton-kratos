...
export namespace LLM {
  ...
  export const OUTPUT_TOKEN_MAX = Flag.KRATOS_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
  ...
  export async function stream(input: StreamInput) {
    ...
    return streamText({
      ...
      headers: {
        ...(input.model.providerID.startsWith("kratos")
          ? {
              "x-kratos-project": Instance.project.id,
              "x-kratos-session": input.sessionID,
              "x-kratos-request": input.user.id,
              "x-kratos-client": Flag.KRATOS_CLIENT,
            }
          : undefined),
        ...input.model.headers,
      },
