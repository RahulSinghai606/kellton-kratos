...
import type { OpencodeClient, SessionMessageResponse } from "@kellton-kratos/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `kratos auth login` in the terminal",
        name: "Login with kratos",
        id: "kratos-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const kratosProvider = providers.find((p) => p.id === "kratos")
    if (kratosProvider) {
      if (kratosProvider.models["big-pickle"]) {
        return { providerID: "kratos", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(kratosProvider.models))
      if (best) {
        return {
