...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kratos.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kratos sessions
...
kratos acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via kratos's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kratos acp
```
...
### Mapping to Kellton Kratos

ACP sessions map cleanly to kratos's internal session model:
