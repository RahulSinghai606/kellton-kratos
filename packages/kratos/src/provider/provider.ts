...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from kratos.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://kellton-kratos.io/",
            "X-Title": "kratos",
          },
        },
      }
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "kratos",
          },
        },
      }
    },
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of Object.entries(providers)) {
      ...
      for (const [modelID, model] of Object.entries(provider.models)) {
        ...
        if (model.status === "alpha" && !Flag.KRATOS_ENABLE_EXPERIMENTAL_MODELS) delete provider.models[modelID]
        if (
  ...
  export async function getSmallModel(providerID: string) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("kratos")) {
        priority = ["gpt-5-nano"]
      }
    ...
    // Check if kratos provider is available before using it
    const kratosProvider = await state().then((state) => state.providers["kratos"])
    if (kratosProvider && kratosProvider.models["gpt-5-nano"]) {
      return getModel("kratos", "gpt-5-nano")
    }
