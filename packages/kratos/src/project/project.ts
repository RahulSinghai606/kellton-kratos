  ...
  export async function fromDirectory(directory: string) {
    ...
    const { id, sandbox, worktree, vcs } = await iife(async () => {
      ...
      if (git) {
        ...
        let id = await Bun.file(path.join(git, "kratos"))
          .text()
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!gitBinary) {
          return {
            ...
            worktree: sandbox,
            sandbox: sandbox,
            vcs: Info.shape.vcs.parse(Flag.KRATOS_FAKE_VCS),
          }
        }
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Bun.file(path.join(git, "kratos"))
              .write(id)
              .catch(() => undefined)
          }
        }
    ...
    if (Flag.KRATOS_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
    const result: Info = {
      ...existing,
      worktree,
      vcs: vcs as Info["vcs"],
