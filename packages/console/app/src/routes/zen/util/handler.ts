...
import { and, Database, eq, isNull, lt, or, sql } from "@kellton-kratos/console-core/drizzle/index.js"
import { KeyTable } from "@kellton-kratos/console-core/schema/key.sql.js"
import { BillingTable, UsageTable } from "@kellton-kratos/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@kellton-kratos/console-core/util/price.js"
import { Identifier } from "@kellton-kratos/console-core/identifier.js"
import { Billing } from "@kellton-kratos/console-core/billing.js"
import { Actor } from "@kellton-kratos/console-core/actor.js"
import { WorkspaceTable } from "@kellton-kratos/console-core/schema/workspace.sql.js"
import { ZenData } from "@kellton-kratos/console-core/model.js"
import { BlackData } from "@kellton-kratos/console-core/black.js"
import { UserTable } from "@kellton-kratos/console-core/schema/user.sql.js"
import { ModelTable } from "@kellton-kratos/console-core/schema/model.sql.js"
import { ProviderTable } from "@kellton-kratos/console-core/schema/provider.sql.js"
...
export async function handler(
  ...
  const FREE_WORKSPACES = [
    ...
    "wrk_01K6W1A3VE0KMNVSCQT43BG2SX", // kratos bench
  ]
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-kratos-session") ?? ""
    const requestId = input.request.headers.get("x-kratos-request") ?? ""
    const projectId = input.request.headers.get("x-kratos-project") ?? ""
    const ocClient = input.request.headers.get("x-kratos-client") ?? ""
    logger.metric({
      is_tream: isStream,
      session: sessionId,
      request: requestId,
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetch(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-kratos-request")
          headers.delete("x-kratos-session")
          headers.delete("x-kratos-project")
          headers.delete("x-kratos-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo) {
    ...
    if (!billing.paymentMethodID)
      throw new CreditsError(
        `No payment method. Add a payment method here: https://kellton-kratos.io/workspace/${authInfo.workspaceID}/billing`,
      )
    if (billing.balance <= 0)
      throw new CreditsError(
        `Insufficient balance. Manage your billing here: https://kellton-kratos.io/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new MonthlyLimitError(
        `Your workspace has reached its monthly spending limit of $${billing.monthlyLimit}. Manage your limits here: https://kellton-kratos.io/workspace/${authInfo.workspaceID}/billing`,
      )
    ...
    if (
      ...
      throw new UserLimitError(
        `You have reached your monthly spending limit of $${authInfo.user.monthlyLimit}. Manage your limits here: https://kellton-kratos.io/workspace/${authInfo.workspaceID}/members`,
      )
  }
