...
import { createOpencode, type ToolPart } from "@kellton-kratos/sdk"
...
console.log("🚀 Starting kratos server...")
const kratos = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await kratos.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new kratos session...")
    const { client, server } = kratos
    ...
    console.log("✅ Created kratos session:", createResult.data.id)
  ...
  console.log("📝 Sending to kratos:", message.text)
