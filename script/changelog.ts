...
import { $ } from "bun"
import { createOpencode } from "@kellton-kratos/sdk"
...
export async function getLatestRelease() {
  return fetch("https://api.github.com/repos/RahulSinghai606/kellton-kratos/releases/latest")
    .then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/RahulSinghai606/kellton-kratos/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/kratos packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/kratos/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/kratos/")) areas.add("core")
...
async function summarizeCommit(kratos: Awaited<ReturnType<typeof createOpencode>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await kratos.client.session.create()
  const result = await kratos.client.session
    .prompt({
      path: { id: session.data!.id },
      body: {
        model: { providerID: "kratos", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], kratos: Awaited<ReturnType<typeof createOpencode>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(kratos, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/RahulSinghai606/kellton-kratos/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const kratos = await createOpencode({ port: 5044 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, kratos)
    notes.push(...lines)
